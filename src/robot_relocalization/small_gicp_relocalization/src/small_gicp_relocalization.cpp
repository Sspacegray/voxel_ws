  ...
  previous_result_t_(Eigen::Isometry3d::Identity())
{
  ...
  this->declare_parameter("pointcloud_topic", "/livox/lidar/pointcloud");  // 更新参数为新的话题名称
  this->declare_parameter("radius", 10.0);  // 声明 radius 参数
  this->declare_parameter("z_min", -2.0);   // 声明 z_min 参数
  this->declare_parameter("z_max", 2.0);    // 声明 z_max 参数
  this->declare_parameter("transform_future_offset", 0.7);  // 声明 transform_future_offset 参数
  ...
  this->get_parameter("pointcloud_topic", pointcloud_topic_);       // 获取参数值
  this->get_parameter("radius", radius_);  // 获取 radius 参数值
  this->get_parameter("z_min", z_min_);    // 获取 z_min 参数值
  this->get_parameter("z_max", z_max_);    // 获取 z_max 参数值
  this->get_parameter("transform_future_offset", transform_future_offset_);  // 获取 transform_future_offset 参数值
  ...
  pcd_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
    pointcloud_topic_, 10,  // 使用参数值替代硬编码话题名称
  ...
  const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
  ...
  // 获取参数 radius, z_min, z_max
  double radius, z_min, z_max;
  ...
  for (const auto &point : scan->points) {
    ...
    if (distance <= radius && point.z >= z_min && point.z <= z_max) {
      filtered_scan->points.push_back(point);
    }
  }

  *accumulated_cloud_ += *filtered_scan;
}
...
void SmallGicpRelocalizationNode::publishTransform()
{
  ...
  double transform_future_offset;
  this->get_parameter("transform_future_offset", transform_future_offset);  // 获取参数值

  geometry_msgs::msg::TransformStamped transform_stamped;
  transform_stamped.header.stamp = this->now() + rclcpp::Duration::from_seconds(transform_future_offset);
