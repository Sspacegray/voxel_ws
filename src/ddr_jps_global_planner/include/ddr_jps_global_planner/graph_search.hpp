...
namespace JPS
{
  ...
  class GraphSearch
  {
    ...
    //   GraphSearch(const char* cMap, int xDim, int yDim, int zDim, double eps = 1, bool verbose = false);
      ...
      explicit GraphSearch(std::shared_ptr<ddr_navigation::CostmapAdapter> map, const double & safe_dis);
       ...
       * @param maxExpand maximum number of expansion allowed, optional, default is -1, means no limitation
       */
      bool plan(int xStart, int yStart, int zStart, int xGoal, int yGoal, int zGoal, bool useJps, int maxExpand = -1) = delete;
    ...
    private:
      ...
      // const char* cMap_;
      std::shared_ptr<ddr_navigation::CostmapAdapter> map_;
