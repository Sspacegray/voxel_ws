#include "pcd2elevation/pcd2elevation.hpp"
#include "pcd2elevation/color_txt.hpp"
...
PCD2Elevation::PCD2Elevation(const rclcpp::NodeOptions& options) : Node("pcd2elevation_node"),  map_(grid_map::GridMap({"elevation"})),
  ...
  filterChain_("grid_map::GridMap")
{
    ...
    Timer_ = rclcpp::create_timer(this,
                                ...
                                std::bind(&PCD2Elevation::PCD2GridMap, this));
...
void PCD2Elevation::loadPCDFile(const std::string& filepath)
{
    pcl::io::loadPCDFile<pcl::PointXYZ>(filepath,*pcd_);

}

void PCD2Elevation::passThroughFilter(double thre_low, double thre_high, bool flag_in)
{
...
void PCD2Elevation::radiusOutlierFilter(const pcl::PointCloud<pcl::PointXYZ>::Ptr& input_cloud, double radius, int thre_count)
{
    auto filtered_cloud = std::make_shared<pcl::PointCloud<pcl::PointXYZ>>();
...
void PCD2Elevation::PCD2GridMap()
{
...
int main(int argc, char** argv)
{
    ...
    auto node = std::make_shared<PCD2Elevation>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}