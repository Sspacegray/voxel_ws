...
void WaypointEditorPanel::onAutoToggle(bool checked)
{
    ...
    try {
        ...
        postStatusMessage(tr("Set parameters failed: %1").arg(ex.what()));
        QSignalBlocker blocker(auto_toggle_button_);
        auto_toggle_button_->setChecked(false);
...
void WaypointEditorPanel::onExecuteWaypointsButtonClick()
{
    if (!execute_client_->wait_for_service(std::chrono::seconds(1))) {
        return;
    }
    auto req = std::make_shared<std_srvs::srv::Trigger::Request>();
    execute_client_->async_send_request(req,
        [this](rclcpp::Client<std_srvs::srv::Trigger>::SharedFuture future) {
            bool ok = false;
            std::string msg;
            try {
            ...
            postStatusMessage(ok ? tr("Execution started") : QString::fromStdString(msg));
        });
}
